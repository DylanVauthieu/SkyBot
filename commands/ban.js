...
module.exports = {
    ...
    async run(bot, message, args) {
        try {
            ...
            if (!reason) reason = "No reason given for the ban."

            if ((await message.guild.fetchOwner()).id === user.id) return message.reply("You can't ban the Owner.")
            if (message.user.id === user.id) return message.reply("Why you want to ban yourself ?")
            if (member && !member.bannable) return message.reply("I can't ban this user.")
            if (member && message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0) return message.reply("You can't ban this user.")
            if ((await message.guild.bans.fetch()).get(user.id)) return message.reply("This user are already ban.")
